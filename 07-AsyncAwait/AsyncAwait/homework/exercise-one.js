...
async function problemA () {
  ...
  */
  ...
  // AsyncAwait version
  let promisa = await promisifiedReadFile('poem-one/stanza-01.txt')
  blue(promisa)
...
*/
  // AsyncAwait version
  let promisa = await promisifiedReadFile('poem-one/stanza-02.txt')
  blue(promisa);
  let promisb = await promisifiedReadFile('poem-one/stanza-03.txt')
  blue(promisb)
}
...
*/
  // AsyncAwait version
  try {
    let promisa = await promisifiedReadFile('poem-one/wrong-file-name.txt')
    blue(promisa);
  } catch (error) {
    magenta(error)
  }
...
*/
  // AsyncAwait version
  try {
    let promisa = await promisifiedReadFile('poem-one/stanza-03.txt')
    blue(promisa);
  } catch (error) {
    magenta(error)
  }
  try {
    let promisb = await promisifiedReadFile('poem-one/wrong-stanza-03.txt')
    blue(promisb);
  } catch (error) {
    magenta(error)
  }
